"""Deterministic, standard-library-only Yakherd 3 installer and transaction engine."""
...
INSTALL_MANIFEST_NAME = "YAKHERD_INSTALL.json"
RETROFIT_LOCK_NAME = ".yakherd-migrate.lock"
RETROFIT_TXN_PREFIX = ".yakherd-migrate-txn-"
FRESH_LOCK_NAME = ".yakherd-setup.lock"
...
def load_package_manifest() -> dict[str, Any]:
    ...
    if release.get("bootstrap_sha256") != sha256_file(PACKAGE_ROOT / "bootstrap.py"):
        raise BootstrapError("release bootstrap hash mismatch")
    if release.get("manifest_sha256") != sha256_file(PACKAGE_MANIFEST_PATH):
        raise BootstrapError("release manifest hash mismatch")
    ...
    if data.get("package_name") != "yakherd_ssot" or data.get("package_version") != "3.0.0":
        raise BootstrapError("unexpected package identity")
    if any(data.get(k) != release.get(k) for k in ("package_name", "package_version")):
        raise BootstrapError("release/manifest identity mismatch")
    if data.get("template_root") != "template":
        raise BootstrapError("invalid template root")
...
def render_template(source: bytes, project_name: str, install_date: str) -> bytes:
    try:
        ...
        raise BootstrapError("all templates must be UTF-8 text") from exc
    replacements = {
...
def make_install_manifest(
    ...
    record = {
        ...
        "files": records,
        "limitations": [
            "no product execution or semantic interpretation of replacement content",
            "no network access",
...
def write_fresh(
    ...
    try:
        ...
        try:
            os.write(lock_fd, b"Yakherd fresh install lock\n")
            os.fsync(lock_fd)
        finally:
            os.close(lock_fd)
            lock_fd = None
...
def write_retrofit(
    ...
    try:
        os.write(lock_fd, b"Yakherd migration transaction lock\n")
        os.fsync(lock_fd)
        os.close(lock_fd)
        lock_fd = -1
    ...
    journal["written_states"] = written_states
    ...
    try:
        ...
        write_json_atomic(journal_path, journal)
        if retain_backups:
            archive = target / f".yakherd-backup-{transaction_id}"
            # Keep exact original bytes and the completed journal for recovery.
            transaction_dir.rename(archive)
            return archive
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--mode", choices=("fresh",), default="fresh",
                        help="fresh only; existing repositories use yakherd migrate")
...
def run(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        preview = {
            ...
            "target": str(target),
            "project_name": project_name,
            "files": selected + [INSTALL_MANIFEST_NAME],
        }
        if args.dry_run:
