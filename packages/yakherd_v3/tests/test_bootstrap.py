...
SPEC = importlib.util.spec_from_file_location("yakherd_v3_bootstrap", BOOTSTRAP_PATH)
...
class BootstrapTests(unittest.TestCase):
    ...
    def test_manifest_matches_template_tree(self) -> None:
        ...
        self.assertEqual(len(manifest["template_files"]), 10)
    ...
    def test_fresh_install_hashes_and_generated_validation(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE_ROOT.parent) as temp:
            ...
            self.assertIn("one implementation task", start_here.lower())
            ...
            self.assertFalse((target / "docs/prompts/codex_team_launcher.md").exists())
            self.assertTrue((target / "NOW.md").is_file())
            self.assertTrue((target / "BASELINE.md").is_file())
            ...
            checked = subprocess.run(
                [sys.executable, "-B", str(PACKAGE_ROOT.parents[1] / "yakherd.py"), "doctor", str(target)],
    ...
    def test_second_fresh_install_refuses_overwrite(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE_ROOT.parent) as temp:
            ...
            original = sha256(target / "BASELINE.md")
            self.assertNotEqual(0, self.run_quiet(args))
            self.assertEqual(original, sha256(target / "BASELINE.md"))
                ...
                json.dumps(
                    {
                        ...
                        "allowed_files": ["BASELINE.md"],
                        "expected_existing_sha256": {"BASELINE.md": sha256(status)},
                    }
                ),
                encoding="utf-8",
            )
    ...
    def test_reviewed_legacy_plan_cannot_select_v3_bootstrap_retrofit(self) -> None:
        with tempfile.TemporaryDirectory(dir=PACKAGE_ROOT.parent) as temp:
            target = Path(temp) / "project"
            target.mkdir()
            readme = target / "README.md"
            ...
            self.assertEqual({"README.md", "product.txt"}, {p.name for p in target.iterdir()})
        ...
        for fail_later in (False, True):
            with self.subTest(fail_later=fail_later), tempfile.TemporaryDirectory(dir=PACKAGE_ROOT.parent) as temp:
                ...
                def tamper(phase: str) -> None:
                    ...
                    if fail_later and phase == "before_commit":
